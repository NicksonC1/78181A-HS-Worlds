...
void set_up(){
		...
		inertial_Deg = lv_label_create(lv_scr_act());
		lv_obj_set_pos(inertial_Deg, 329, 215); //325
...
void screen_upd(){
	while(1){
		...
		LVGL_vals:: lbAvg = (Motor::lbL.get_temperature() + Motor::lbR.get_temperature())/2.0;
		LVGL_vals:: intake = Motor::intake.get_temperature();

		if((RMotor[0])/40.0 >= 1){
			Rtemp = 100;
		} else Rtemp = (int)(RMotor[0]/40);

		if((LMotor[0])/40.0 >= 1){
			Rtemp = 100;
		} else Ltemp = (int)(LMotor[0]/40);

		if((LVGL_vals::intake)/40.0 >= 1){
			intake = 100;
		} else intake = (int)(intake/40.0);

		if((LVGL_vals::lbAvg)/40.0 >= 1){
			LVGL_vals::lbAvg = 100;
		} else lb_temp = (int)(LVGL_vals::lbAvg/40.0);


		lv_bar_set_value(bar1, (int)(Ltemp/40.0), LV_ANIM_OFF);
		lv_bar_set_value(bar2, (int)(Rtemp/40.0), LV_ANIM_OFF);
		lv_bar_set_value(bar3, (int)(intake/40.0), LV_ANIM_OFF);
		lv_bar_set_value(bar4, (int)(lb_temp/40.0), LV_ANIM_OFF);
		lv_bar_set_value(Bat_rec_indicator, LVGL_vals::bVolt, LV_ANIM_OFF);

		//Auton Selector UPD
		lv_roller_set_selected(autonroller, 1, LV_ANIM_OFF);

		//Label Update
		lv_label_set_text(inertial_Deg, "Arm");
