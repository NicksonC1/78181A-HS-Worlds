...
float lebron::slew(float target, float current, float maxChange) {
...
constexpr float lebron::sanitizeAngle(float angle, bool radians) {
    if (radians) return std::fmod(std::fmod(angle, 2 * M_PI) + 2 * M_PI, 2 * M_PI);
    else return std::fmod(std::fmod(angle, 360) + 360, 360);
}

float lebron::angleError(float target, float position, bool radians, AngularDirection direction) {
...
float lebron::avg(std::vector<float> values) {
    float sum = 0;
    for (float value : values) { sum += value; }
    return sum / values.size();
}

float lebron::ema(float current, float previous, float smooth) {
    return (current * smooth) + (previous * (1 - smooth));
}

float lebron::getCurvature(Pose pose, Pose other) {
    // calculate whether the pose is on the left or right side of the circle
    float side = lebron::sgn(std::sin(pose.theta) * (other.x - pose.x) - std::cos(pose.theta) * (other.y - pose.y));
