...
// global variables
lebron::OdomSensors odomSensors(nullptr, nullptr, nullptr, nullptr, nullptr); // the sensors to be used for odometry
lebron::Drivetrain drive(nullptr, nullptr, 0, 0, 0, 0); // the drivetrain to be used for odometry
lebron::Pose odomPose(0, 0, 0); // the pose of the robot
lebron::Pose odomSpeed(0, 0, 0); // the speed of the robot
lebron::Pose odomLocalSpeed(0, 0, 0); // the local speed of the robot
...
void lebron::setSensors(lebron::OdomSensors sensors, lebron::Drivetrain drivetrain) {
    odomSensors = sensors;
    drive = drivetrain;
}

lebron::Pose lebron::getPose(bool radians) {
    if (radians) return odomPose;
    else return lebron::Pose(odomPose.x, odomPose.y, radToDeg(odomPose.theta));
}

void lebron::setPose(lebron::Pose pose, bool radians) {
    if (radians) odomPose = pose;
    else odomPose = lebron::Pose(pose.x, pose.y, degToRad(pose.theta));
}

lebron::Pose lebron::getSpeed(bool radians) {
    if (radians) return odomSpeed;
    else return lebron::Pose(odomSpeed.x, odomSpeed.y, radToDeg(odomSpeed.theta));
}

lebron::Pose lebron::getLocalSpeed(bool radians) {
    if (radians) return odomLocalSpeed;
    else return lebron::Pose(odomLocalSpeed.x, odomLocalSpeed.y, radToDeg(odomLocalSpeed.theta));
}

lebron::Pose lebron::estimatePose(float time, bool radians) {
...
void lebron::update() {
    ...
    lebron::TrackingWheel* verticalWheel = nullptr;
    lebron::TrackingWheel* horizontalWheel = nullptr;
