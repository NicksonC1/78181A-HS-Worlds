 ...
 * @return std::vector<lebron::Pose> vector of points on the path
 */
std::vector<lebron::Pose> getData(const asset& path) {
    std::vector<lebron::Pose> robotPath;
    ...
    for (std::string line : dataLines) {
        lebron::infoSink()->debug("read raw line {}", stringToHex(line));
        ...
        if (pointInput.size() != 3) {
            lebron::infoSink()->error("Failed to read path file! Are you using the right format? Raw line: {}",
                                      stringToHex(line));
            break;
        }
        lebron::Pose pathPoint(0, 0);
        ...
        lebron::infoSink()->debug("read point {}", pathPoint);
    }
 ...
 */
int findClosest(lebron::Pose pose, std::vector<lebron::Pose> path) {
    int closestPoint;
 ...
 */
float circleIntersect(lebron::Pose p1, lebron::Pose p2, lebron::Pose pose, float lookaheadDist) {
    // calculations
    // uses the quadratic formula to calculate intersection points
    lebron::Pose d = p2 - p1;
    lebron::Pose f = p1 - pose;
    float a = d * d;
    float b = 2 * (f * d);
 ...
 */
lebron::Pose lookaheadPoint(lebron::Pose lastLookahead, lebron::Pose pose, std::vector<lebron::Pose> path, int closest,
                            float lookaheadDist) {
    ...
    for (int i = start; i < path.size() - 1; i++) {
        lebron::Pose lastPathPose = path.at(i);
        lebron::Pose currentPathPose = path.at(i + 1);

        float t = circleIntersect(lastPathPose, currentPathPose, pose, lookaheadDist);

        if (t != -1) {
            lebron::Pose lookahead = lastPathPose.lerp(currentPathPose, t);
            lookahead.theta = i;
            return lookahead;
        }
    }
 ...
 */
float findLookaheadCurvature(lebron::Pose pose, float heading, lebron::Pose lookahead) {
    // calculate whether the robot is on the left or right side of the circle
    float side = lebron::sgn(std::sin(heading) * (lookahead.x - pose.x) - std::cos(heading) * (lookahead.y - pose.y));
...
void lebron::Chassis::follow(const asset& path, float lookahead, int timeout, bool forwards, bool async) {
    ...
    std::vector<lebron::Pose> pathPoints = getData(path); // get list of path points
