...
lebron::TrackingWheel::TrackingWheel(pros::adi::Encoder* encoder, float wheelDiameter, float distance,
                                     float gearRatio) {
...
lebron::TrackingWheel::TrackingWheel(pros::Rotation* encoder, float wheelDiameter, float distance, float gearRatio) {
...
lebron::TrackingWheel::TrackingWheel(pros::MotorGroup* motors, float wheelDiameter, float distance, float rpm) {
    this->motors = motors;
...
void lebron::TrackingWheel::reset() {
...
float lebron::TrackingWheel::getDistanceTraveled() {
    if (this->encoder != nullptr) {
        ...
        return lebron::avg(distances);
    } else {
        return 0;
    }
}

float lebron::TrackingWheel::getOffset() { return this->distance; }

int lebron::TrackingWheel::getType() {
